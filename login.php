...
// Login de dev
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tipo']) && $_POST['tipo'] === 'dev') {
    $login = trim($_POST['login'] ?? '');
    $senha = $_POST['senha'] ?? '';

    $stmt = $conn->prepare("SELECT id, nome, email, senha, nivel FROM devs WHERE email = ?");
    $stmt->bind_param("s", $login);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result && $result->num_rows === 1) {
        ...
        if (password_verify($senha, $dev['senha'])) {
            $_SESSION['usuario_id'] = $dev['id'];
            $_SESSION['usuario'] = $dev['nome'];
            $_SESSION['tipo'] = 'dev';
            $_SESSION['nivel'] = $dev['nivel']; // Salva o nível do dev
        ...
        $erro_usuario = "Desenvolvedor não encontrado.";
    }
    $stmt->close();
}
...
<body>
    <div class="login-box">
        <div class="tabs">
            ...
            <div class="tab" onclick="showTab('dev')">Desenvolvedor</div>
        </div>
        ...
        <!-- Formulário Dev -->
        <div id="dev" class="form-section">
            <?php if ($erro_dev): ?><div class="erro"><?= $erro_dev ?></div><?php endif; ?>
            <form method="POST">
